...
class GameModel: NSObject {
    ...
    init(tileNumber t: Int, winningScore w: Int, delegate: GameModelProtocol) {
        tileNumber = t
        winningScore = w
        self.delegate = delegate
        queue = [MoveCommand]()
        timer = NSTimer()
        gameboard = SquareGameboard(tileNumber: t, initialValue: .Empty)
        super.init()
    }
    ...
    func userHasWon() -> (Bool, (Int, Int)?) {
        for i in 0..<tileNumber {
            for j in 0..<tileNumber {
                if case let .Tile(v) = gameboard[i, j] where v >= winningScore {
                    return (true, (i, j))
                }
            }
        }
    ...
    func performMove(direction: MoveDirection) -> Bool {
        ...
        let coordinateGenerator: (Int) -> [(Int, Int)] = { (iteration: Int) -> [(Int, Int)] in
            var buffer = Array<(Int, Int)>(count: self.tileNumber, repeatedValue: (0, 0))
            for i in 0..<self.tileNumber {
                switch direction {
                case .Up:
                    buffer[i] = (i, iteration)
                case .Down:
                    buffer[i] = (self.tileNumber - i - 1, iteration)
                case .Left:
                    buffer[i] = (iteration, i)
                case .Right:
                    buffer[i] = (iteration, self.tileNumber - i - 1)
                }
            }
            return buffer
        }
        ...
        for i in 0..<tileNumber {
            ...
            for object in orders {
                switch object {
                case let MoveOrder.SingleMoveOrder(soure: s, destination: d, value: v, wasMerge: wasMerge):
                    let (sx, sy) = coords[s]
                    let (dx, dy) = coords[d]
                    if wasMerge {
                        score += v
    ...
    func convert(group: [ActionToken]) -> [MoveOrder] {
        ...
        for (idx, t) in group.enumerate() {
            switch t {
            case let .Move(s, v):
                moveBuffer.append(MoveOrder.SingleMoveOrder(soure: s, destination: idx, value: v, wasMerge: false))
            case let .SingleCombine(s, v):
                moveBuffer.append(MoveOrder.SingleMoveOrder(soure: s, destination: idx, value: v, wasMerge: true))
